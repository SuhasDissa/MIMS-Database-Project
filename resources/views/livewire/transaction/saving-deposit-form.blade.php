    ...
    <x-mary-form wire:submit="submit">
        ...
        <div class="col-span-1 md:col-span-2">
            ...
            <div class="flex gap-2">
                <div class="flex-1">
                    <x-mary-input wire:model.live.debounce.300ms="account_search" wire:model="account_number"
                        placeholder="Search by account number or customer name" list="accounts-list" required
                        class="w-full" />
                    <datalist id="accounts-list">
                        @foreach ($filteredAccounts as $acc)
                            <option value="{{ $acc->account_number }}">
                                {{ $acc->account_number }} - {{ $acc->customers->first()->first_name ?? 'N/A' }}
                                {{ $acc->customers->first()->last_name ?? '' }}
                            </option>
                        @endforeach
                    </datalist>
                </div>
                <x-mary-button wire:click="searchAccount" icon="o-magnifying-glass" class="btn-primary" type="button"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    @if ($account_balance !== null)
                        <div>
                            ...
                            <span class="font-semibold text-success ml-2">Rs.
                                {{ number_format($account_balance, 2) }}</span>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <x-mary-input label="Amount (Rs.)" wire:model="amount" type="number" min="0" step="0.01" required
            class="w-full" />
        <div class="col-span-1 md:col-span-2">
        ...
        <x-slot:actions>
            <div class="flex justify-center mt-6 w-full">
                <x-mary-button label="Deposit Money" class="w-100 btn-primary" type="submit" />
            </div>
        </x-slot:actions>
    </x-mary-form>
</div>
